...
pub async fn check_urls(client: &Client, metadata_url: &str) -> Result<bool> {
    ...
    let is_valid_url = |url_str: &str| -> bool { Url::parse(url_str).is_ok() };

    // Check for Twitter URL
    let has_twitter = match data.get("twitter") {
        Some(twitter) => match twitter.as_str() {
            Some(twitter_str) if !twitter_str.trim().is_empty() => is_valid_url(twitter_str),
            _ => false,
        },
        None => false,
    };
    ...
    let has_telegram = match data.get("telegram") {
        Some(telegram) => match telegram.as_str() {
            Some(telegram_str) if !telegram_str.trim().is_empty() => is_valid_url(telegram_str),
            _ => false,
        },
        None => false,
    };
    ...
    let has_website = match data.get("website") {
        Some(website) => match website.as_str() {
            Some(website_str) if !website_str.trim().is_empty() => is_valid_url(website_str),
            _ => false,
        },
        None => false,
    };
...
#[inline]
pub async fn check_minimum_liquidity(
    client: &Client,
    dev_wallet: &str,
    mint: &str,
) -> Result<bool> {
    ...
    let balance = api::retry_async(
        || async { api::get_balance(client, dev_wallet, mint).await },
        Some(2),    // max retries
    ...
    let sol_deposited =
        (constant / (initial_virtual_token_reserve as f64 - dev_token_balance as f64)) - offset;

    // Check against the minimum liquidity threshold
    if sol_deposited >= min_liquidity {
        info!(
            "Token {} meets minimum liquidity requirements ({:.2} SOL >= {} SOL)",
