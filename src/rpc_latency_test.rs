...
pub async fn test_rpc_latency(endpoint: &str, auth_header: Option<(&str, String)>) -> Result<Duration> {
...
pub async fn find_fastest_rpc() -> Result<String> {
    ...
    if !bloxroute_auth_token.is_empty() && bloxroute_auth_token != "BLOXROUTE_AUTH_TOKEN" {
        ...
        endpoints.push(("https://solana.api.blxrbdn.com", Some(("Authorization", auth_header))));
    }
    ...
    println!("\nFastest RPC endpoint: {} with latency: {:?}", fastest_endpoint, fastest_time);
    ...
    if fastest_endpoint.contains("helius") {
        ...
        println!("Add this to your .env file as RPC_URL={} with Authorization header", fastest_endpoint);
    } else {
        println!("Add this to your .env file as RPC_URL={}", fastest_endpoint);
    }
...
pub async fn compare_all_endpoints() -> Result<()> {
    ...
    let use_bloxroute = !bloxroute_auth_token.is_empty() && bloxroute_auth_token != "BLOXROUTE_AUTH_TOKEN";
    ...
    let mut stats = vec![
        EndpointStats { name: "Solana Standard".to_string(), total_latency: Duration::from_secs(0), success_count: 0 },
        EndpointStats { name: "Helius".to_string(), total_latency: Duration::from_secs(0), success_count: 0 },
        EndpointStats { name: "Pumpfun".to_string(), total_latency: Duration::from_secs(0), success_count: 0 },
        EndpointStats { name: "Pumpfun bloXroute".to_string(), total_latency: Duration::from_secs(0), success_count: 0 },
    ];
    ...
    for stat in &stats {
        if stat.success_count > 0 {
            ...
            println!("{}: {:.2}ms (from {} successful tests)", stat.name, avg, stat.success_count);
    ...
    if !fastest_name.is_empty() {
        println!("\nFASTEST ENDPOINT: {} with average latency of {:.2}ms", fastest_name, fastest_avg);
        ...
        match fastest_name.as_str() {
            ...
            "bloXroute Direct" => println!("Recommended endpoint: {} (with Authorization header)", bloxroute_endpoint),
            _ => {}
        }
    }
...
pub async fn compare_helius_vs_bloxroute() -> Result<()> {
    ...
    if helius_success > 0 {
        ...
        println!("\nHelius average latency: {:.2}ms (from {} successful tests)", helius_avg, helius_success);
    } else {
    ...
    if bloxroute_success > 0 {
        ...
        println!("bloXroute average latency: {:.2}ms (from {} successful tests)", bloxroute_avg, bloxroute_success);
    } else {
    ...
    if helius_success > 0 && bloxroute_success > 0 {
        ...
        if helius_avg < bloxroute_avg {
            println!("\nRESULT: Helius is faster by {:.2}ms", bloxroute_avg - helius_avg);
            println!("Recommended endpoint: {}", helius_endpoint);
        } else {
            println!("\nRESULT: bloXroute is faster by {:.2}ms", helius_avg - bloxroute_avg);
            println!("Recommended endpoint: {} (with Authorization header)", bloxroute_endpoint);
        }
    }
